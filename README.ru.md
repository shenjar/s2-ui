...
> Требуется, чтобы TCP-порт **80** был доступен из интернета (HTTP-01 challenge; при использовании
> Docker опубликуйте `-p 80:80`). Сертификаты хранятся в каталоге `cert/` и сохраняются при
